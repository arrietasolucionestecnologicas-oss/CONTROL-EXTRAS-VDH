/** APP.JS V15.0 - PROFESSIONAL SUITE */
const CONFIG = {
...
let S = JSON.parse(sessionStorage.getItem('vdh_v15')) || { dbId: null, role: null, viewRole: null };
...
const api = async (act, pl={}) => {
    ...
    try {
        ...
        console.error(e); 
        return {status:"error", message: "Error de Red: " + e.message}; 
    }
};
...
const app = {
    ...
    login: () => {
        ...
        api("login", {user:u, pass:p}).then(j => {
            ...
            if(j.status==='success') {
                ...
                sessionStorage.setItem('vdh_v15', JSON.stringify(S));
    ...
    modRegistro: () => {
        api("get_full_data").then(j => {
            ...
            fill('sal-trabajador', d.empleados); // Para el tab de salarios
    ...
    updateSalary: () => {
        const t = document.getElementById('sal-trabajador').value;
        const m = document.getElementById('sal-monto').value;
        if(t && m) api("actualizar_salario", {nombre:t, nuevoSalario:m}).then(()=>{ toast("Salario Actualizado"); });
    },
    ...
    modFinanzas: () => {
        api("get_finance").then(j => {
            ...
            if(d.pendientes) d.pendientes.forEach(r => {
                t.innerHTML += `<tr><td>${r.fecha}</td><td>${r.trab}</td><td>${r.total}</td><td>${r.ord}</td><td>${r.rn}</td><td>${r.ed}</td><td>${r.en}</td><td>${r.df}</td><td>${r.edom}</td><td class="text-end fw-bold text-success">$${r.valor.toLocaleString()}</td><td><button class="btn btn-sm btn-success py-0" onclick="app.approve('${r.rowId}')">OK</button></td></tr>`;
            });
    ...
    modAdmin: () => {
        api("get_public_list").then(j => {
            ...
            j.data.forEach(e => {
                t.innerHTML += `<tr><td>${e.nombre}</td><td><small>${e.token}</small></td><td><button class="btn btn-sm btn-danger" onclick="app.admDel('${e.id}')">X</button></td></tr>`;
            });
        });
    },
    admCreate: () => {
